...
@app.cell
def _(Image, ImageFilter, np):
    def preprocess_for_mnist(img_pil):
        """Match MNIST's preprocessing: crop, fit-in-20x20, center by center-of-mass."""
        # Smooth jagged mouse strokes
        img = img_pil.filter(ImageFilter.GaussianBlur(1))

        # 1. Crop to the digit's bounding box (drops black margins)
        bbox = img.getbbox()
        if bbox is None:                          # canvas is empty
            return Image.new('L', (28, 28), 0)
        img = img.crop(bbox)

        # 2. Resize so the longer side is 20, preserving aspect ratio
        w, h = img.size
        if w >= h:
        ...
        # 3. Paste onto 28x28 canvas so center-of-mass lands at (14, 14)
        canvas = Image.new('L', (28, 28), 0)
        arr = np.array(img, dtype=np.float32)
        if arr.sum() > 0:
            ys, xs = np.indices(arr.shape)
            com_x = (xs * arr).sum() / arr.sum()
            com_y = (ys * arr).sum() / arr.sum()
            offset = (int(round(14 - com_x)), int(round(14 - com_y)))
        else:
            offset = ((28 - new_w) // 2, (28 - new_h) // 2)
        canvas.paste(img, offset)
        return canvas

    return (preprocess_for_mnist,)
...
def _(
    ...
    img_28 = preprocess_for_mnist(img_pil=img_pil)
    img_tensor = transforms.ToTensor()(img_28).unsqueeze(0)    # [1, 1, 28, 28], normalized [0,1]
